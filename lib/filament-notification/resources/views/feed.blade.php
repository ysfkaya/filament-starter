<div x-data="{
    ...
    <button x-on:click="isOpen = ! isOpen" @class(['flex items-center justify-center w-10 h-10 '])>
        <x-heroicon-o-bell
            class="h-5 -mr-1 align-text-top  @if ($this->totalUnread) animate-swing @endif origin-top" />
        @if ($this->totalUnread)
            <sup
                class="inline-flex items-center justify-center p-1 text-xs leading-none text-white bg-danger-600 rounded-full">
                {{ $this->totalUnread }}
            </sup>
        @endif
    </button>
    ...
    <div x-show="isOpen" x-on:click.away="isOpen = false" x-transition:enter="transition"
        ...
        x-transition:leave-end="-translate-y-1 opacity-0" x-cloak @class([
        ...
        style="min-width: {{ $notifications->isNotEmpty() ? '350px' : '220px' }}"
        >
        @if ($notifications->isNotEmpty())
            <ul @class([
                ...
                @foreach ($notifications as $notification)
                    <li @class(['relative', $notification->read() ? 'opacity-50' : ''])>
                        ...
                        <small
                            class="block px-3 text-sm font-normal">{{ Arr::get($notification->data, 'message') }}</small>
            ...
            <div class="p-2">
                <x-filament-notification::button wire:click="markAllAsRead" :color="config('filament-notification.buttons.markAllRead.color', 'primary')" :outlined="config('filament-notification.buttons.markAllRead.outlined', false)"
                    :icon="config(
                        'filament-notification.buttons.markAllRead.icon',
                        'filament-notification::icon-check-all',
                    )" :size="config('filament-notification.buttons.markAllRead.size', 'sm')" class="w-full mt-2 h-8">
    ...
    <form wire:submit.prevent="callMountedNotificationAction">
        ...
        <x-tables::modal displayClasses="block" :id="\Illuminate\Support\Str::of(static::class)->replace('\\', '\\\\') . '-action'" :width="$action?->getModalWidth()" display-classes="block">
            @if ($action)
