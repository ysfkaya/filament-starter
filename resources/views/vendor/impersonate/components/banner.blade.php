...
@php
$impersonateGuard = config('filament-impersonate.guard');

$style = $style ?? config('filament-impersonate.banner.style');
$display = $display ?? auth($impersonateGuard)->user() ? Filament\Facades\Filament::getUserName(auth($impersonateGuard)->user()) : null;
